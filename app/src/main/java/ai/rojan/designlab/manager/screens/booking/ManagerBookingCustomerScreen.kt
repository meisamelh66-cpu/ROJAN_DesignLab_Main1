    ...
    ManagerGlassSurface(
        ...
        Row(
            ...
            Column(modifier = Modifier.fillMaxWidth()) {
                ...
                // placeholder, same convention ProfileScreen.kt already
                // uses for the identical nullable-email case.
                customer.email?.takeIf { it.isNotBlank() }?.let { email ->
                    Text(text = email, style = RojanTypography.Caption, color = ManagerColors.TextSecondary)
                }
            }
        }
    }
