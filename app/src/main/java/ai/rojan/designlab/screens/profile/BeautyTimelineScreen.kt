...
/**
 * Journey 2, Screen 9: Beauty Timeline.
 *
 * Production Data Integrity Phase 1: gated. No beauty-history endpoint
 * exists anywhere in `data/remote/dto/` or `domain/repository/` — this
 * was entirely demo/in-memory state
 * ([ai.rojan.designlab.data.demo.DemoBeautyTimelineRepository] via
 * [CustomerEcosystemViewModel]). Entry point stays reachable from Profile;
 * content becomes a Coming Soon state until a real beauty-history
 * capability exists on the backend.
 */
@Composable
