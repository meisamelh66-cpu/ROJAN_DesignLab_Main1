 ...
 *
 * [cancel] (branch-integration reconciliation): also real —
 * `BookingController.cancel`, `PATCH /api/v1/bookings/{bookingId}/cancel`,
 * confirmed present against current backend source (this endpoint's own
 * authorization is broader than [confirm]/[complete]'s — it allows either
 * the booking's own customer *or* `Permission.MANAGE_BOOKINGS`, whereas
 * confirm/complete require `MANAGE_BOOKINGS` unconditionally — irrelevant
 * from this Manager-only API surface, since a Manager caller always
 * resolves through the `MANAGE_BOOKINGS` branch, but worth noting since
 * this same endpoint is also called from Customer's own cancel flow via
 * [ai.rojan.designlab.data.remote.BookingApi]). `reschedule` is still not
 * added here: it was not part of any approved scope so far, so its
 * absence (and [ai.rojan.designlab.manager.data.BackendAppointmentRepository]'s
 * pre-existing local-cache-only handling of it) is left exactly as-is.
 */
interface ManagerBookingApi {
    ...
    suspend fun cancel(@Path("bookingId") bookingId: String): BookingResponseDto
}
